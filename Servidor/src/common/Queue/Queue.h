#ifndef TALLERZ_QUEUE_H
#define TALLERZ_QUEUE_H
#include <mutex>
...
class Queue {
 ...
 public:
  Queue() {}
  bool empty() {
    std::unique_lock<std::mutex> lck(mtx); // mtx.lock() pero en modo RAII
    return q.empty();
  }
  ...
  T pop() {
    ...
    while (q.empty()) {
       ...
       *
       * Colorario: el metodo wait debe llamarse solo si el mutex ya fue adquirido
